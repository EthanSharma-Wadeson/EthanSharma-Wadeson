...
ROW_MARGIN = int(os.environ.get("WORDMARK_ROW_MARGIN", 14))
CELL_W = 4.9
CELL_H = 8.6
...
if not os.path.exists(DEFAULT_FONT):
    for candidate, idx in [
        (r"C:\Windows\Fonts\trebucbd.ttf", 0),
        (r"C:\Windows\Fonts\arialbd.ttf", 0),
        (r"C:\Windows\Fonts\seguisb.ttf", 0),
...
TEXT = os.environ.get("WORDMARK_TEXT", "ETHAN")
...
DEPTH_FRAC = 0.18      # extrusion depth (leaner depth keeps counters and strokes legible)
TILT_DEG = float(os.environ.get("WORDMARK_TILT", 3.0))
CAM_DIST = 6.0         # camera distance in world units (1.0 == wordmark width)
FOCAL = 4.15
FIT = 0.94             # fraction of the grid the widest pose may use
...
def main():
    ...
    P, N = build_shell()
    rest = math.radians(-8)                   # front-facing 3/4 pose keeps letters legible
    if a.mode == "spin":
        nf = a.frames or 36
