...
def rings():
    ...
    # inner infinity mark
    out.append(
        f'<text x="{CX:.1f}" y="{ART_Y + 46:.1f}" fill="{CYAN}" font-size="22" '
        f'font-weight="700" text-anchor="middle" letter-spacing="4" opacity="0.85">'
        f"LIMITLESS"
        f'<animate attributeName="opacity" values="0.35;0.95;0.35" dur="3.2s" '
...
def main():
    ...
    title = f"{USER_HANDLE}@github: ~$ ./limitless.sh"
