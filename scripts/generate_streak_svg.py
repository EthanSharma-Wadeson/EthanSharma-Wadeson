...
USER = sys.argv[1] if len(sys.argv) > 1 else os.environ.get("GH_PROFILE_USER", "EthanSharma-Wadeson")
...
def get_data(user):
    ...
    try:
        ...
        for fallback in [
            os.path.join(HERE, "contrib.json"),
            os.path.join(HERE, "..", "data", "contributions.json"),
        ]:
            if os.path.exists(fallback):
                ...
                if "contributions" in raw:
                    ...
                    # adapt data/contributions.json format
                    total = raw.get("total_contributions", 0)
                    days = raw.get("days", [])
                    contribs = []
                    for d in days:
                        cnt = d.get("count", 0)
                        lvl = 0 if cnt == 0 else (1 if cnt <= 3 else (2 if cnt <= 6 else (3 if cnt <= 12 else 4)))
                        contribs.append({"date": d["date"], "count": cnt, "level": lvl})
                    return {"contributions": contribs, "total": {"lastYear": total}}
        raise
